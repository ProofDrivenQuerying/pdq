...
public class ConstraintCardinalityEstimator4  extends ConstraintCardinalityEstimator0{
	...
	protected NormalisedPlan loadPlan() {
		...
		this.projection0 = new ProjectCommand(Lists.newArrayList(attr), this.selection0.getOutput());

		this.access1 = new AccessCommand(schema.getRelation("AssayLimited"), schema.getRelation("AssayLimited").getAccessMethod("chembl_assay_limited_2"), 
		...
		this.join1 = new JoinCommand(this.selection1.getOutput(), this.selection0.getOutput());
		Attribute attr1 = (Attribute) this.join1.getOutput().getHeader().get(21);
		this.projection1 = new ProjectCommand(Lists.newArrayList(attr1), this.join1.getOutput());

		this.access2 = new AccessCommand(this.schema.getRelation("TargetLimited"), this.schema.getRelation("TargetLimited").getAccessMethod("chembl_target_limited"), 
		...
		this.join2 = new JoinCommand(this.access2.getOutput(), this.join1.getOutput());
	...
	public void test10() {
		...
		Assert.assertTrue("Conjunctions must match be equal ", estim2.equals(Pair.of(3822,false)));
	...
	public void test12() {
		...
		Assert.assertTrue("Conjunctions must match be equal ", estim6==1000);
