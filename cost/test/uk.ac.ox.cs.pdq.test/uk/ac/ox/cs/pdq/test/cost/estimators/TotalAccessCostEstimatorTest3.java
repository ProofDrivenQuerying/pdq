...
public class TotalAccessCostEstimatorTest3 {
	...
	@Test
	public void callTestPlan3() throws FileNotFoundException, IOException {
...
//			PublicationShort.setKey(Lists.newArrayList(id));
			...
			this.testPlan3(schema, query, catalog);
		}

	}
	
	//Creates the plan for the configuration of case_018a
	...
	private void testPlan3(Schema schema, Query<?> query, Catalog catalog) {
		...
		Command access0 = new Access(schema.getRelation("TargetFree"), schema.getRelation("TargetFree").getAccessMethod("chembl_target_free"), 
		...
		Command projection0 = new Project(Lists.newArrayList(attr), access0.getOutput());
			
		Command access1 = new Access(schema.getRelation("AssayLimited"), schema.getRelation("AssayLimited").getAccessMethod("chembl_assay_limited_1"), 
		...
		Command join1 = new Join(selection1.getOutput(), access0.getOutput());

		Command access2 = new Access(schema.getRelation("DocumentFree"), schema.getRelation("DocumentFree").getAccessMethod("chembl_document_free"), 
		...
		Command selection2 = new Select(new ConjunctivePredicate(Lists.newArrayList(p20)), access1.getOutput());
		Command join2 = new Join(selection2.getOutput(), join1.getOutput());
		
		NormalisedPlan plan = new NormalisedPlan(Lists.newArrayList(access0, projection0, access1, selection1, join1, access2,
				selection2, join2));
		...
		boolean r4 = estimator.isKey(access0.getOutput(), Lists.newArrayList(k1), CollectionUtils.union(keyDependencies, constraints));
		
		System.out.println();
		
/*		
		//Test keys and inclusion dependencies between the tables
		boolean r1 = estimator.existsInclustionDependency(selection1.getOutput(), access0.getOutput(), constraints);
		boolean r2 = estimator.existsInclustionDependency(access0.getOutput(), selection1.getOutput(), constraints);
