...
public class Relation implements Serializable {
	...
	/**
	 * Primary keys are detected by the schema discovery classes, and stored here,
	 * but currently there is no usage for it. Primary keys could be used to
	 * generate EGDs such that two tuple with the same primary key must be the same
	 * tuple.
	 */
	...
	protected Relation(String name, Attribute[] attributes, AccessMethodDescriptor[] accessMethods,
			ForeignKey[] foreignKeys, boolean isEquality) {
		this(name, attributes, accessMethods, foreignKeys, isEquality, null);
	}

	protected Relation(String name, Attribute[] attributes, AccessMethodDescriptor[] accessMethods,
			ForeignKey[] foreignKeys, boolean isEquality, String[] indexedAttributes) {
		this.name = name;
	 ...
	 *
	 * @return true if the signature is of an equality predicate, false otherwise
	 */
	 ...
	 * @return the index of the attribute whose name is given as parameter, returns
	 *         -1 iff the relation has no such attribute
	 */
	...
	public Integer[] getKeyPositions() {
		...
		for (int attributeIndex = 0; attributeIndex < this.primaryKey.getNumberOfAttributes(); ++attributeIndex)
			keyPositions[attributeIndex] = Arrays.asList(this.attributes)
					.indexOf(this.primaryKey.getAttributes()[attributeIndex]);
		return keyPositions;
	}
	...
	public static Relation create(String name, Attribute[] attributes, AccessMethodDescriptor[] accessMethods,
		...
		return Cache.relation
				.retrieve(new Relation(name, attributes, accessMethods, foreignKeys, isEquality, indexedAttributes) {
					private static final long serialVersionUID = -8215821247702132205L;
				});
	}
