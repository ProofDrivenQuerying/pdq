...
public class TestAccessRepository extends PdqTest{
	...
	@Test
	public void testAccessExport() throws JAXBException {
		...
		Assert.assertEquals(new File("test/schemas/accesses/dbAccessMethodOut.xml").length(), new File("test/schemas/accesses/dbAccessMethod.xml").length());		
		target.close();
	...
	public void testAccessImport() throws JAXBException {
		...
		try {
			target = (DatabaseAccessMethod) DbIOManager.importAccess(new File("test/schemas/accesses/dbAccessMethod.xml"));
		} catch(Throwable t) {
			t.printStackTrace();
			t.printStackTrace();
		}
