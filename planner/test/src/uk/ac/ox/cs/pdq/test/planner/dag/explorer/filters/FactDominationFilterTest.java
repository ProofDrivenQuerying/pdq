...
public class FactDominationFilterTest {
	...
	@Test public void test1() {
		...
		when(config32.getInput()).thenReturn(Lists.<Constant>newArrayList(UntypedConstant.create("c1"), UntypedConstant.create("c2")));

		//java.lang.AssertionError: 
		//expected:<[config12, config21, config31, config11, config22]> 
		//but was:<[config12, config11, config22]>
		
		Assert.assertEquals(Sets.newHashSet(config11, config12, config21, config22, config31),
				this.filter.filter(Sets.newHashSet(config11, config12, config21, config22, config31, config32)));
	}
}
