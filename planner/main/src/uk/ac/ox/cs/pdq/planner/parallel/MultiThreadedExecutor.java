...
import uk.ac.ox.cs.pdq.planner.dag.parallel.ChaseOrPropagateThread;
import uk.ac.ox.cs.pdq.planner.dag.parallel.FinalIterationThread;
...
public class MultiThreadedExecutor extends IterativeExecutor {
	...
	public Collection<DAGChaseConfiguration> chaseOrPropagate(
		...
		Map<DAGChaseConfiguration, Integer> participatesIn = new ConcurrentHashMap<>();
		...
		try {
			...
			do {
				...
				priority.getPotential().update(bestConfiguration == null ? null : bestConfiguration.getPlan());
				for(int j = 0; j < this.mtcontext.getParallelThreads(); ++j) {
					...
					threads.add(new ChaseOrPropagateThread(
							depth,
							leftInput,
							rightInput,
							priority.clone(),
