...
import uk.ac.ox.cs.pdq.cardinality.logging.IntervalEventDrivenLogger;
...
public class Bootstrap {
	 ...
	 */
	public void run() {
		CardinalityParameters planParams = this.getConfigFile() != null ?
				new CardinalityParameters(this.getConfigFile()) :
				new CardinalityParameters() ;
		...
		try(FileInputStream sis = new FileInputStream(this.getSchemaPath());
			...
			try(ProgressLogger pLog = new SimpleProgressLogger(System.out)) {
				Cardinality planner = new Cardinality(planParams, costParams, reasoningParams, schema, query);
				if (verbose) {
