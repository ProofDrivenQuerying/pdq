...
public abstract class DAGAnnotatedPlan extends AnnotatedPlan<DAGPlan> implements DAGConfiguration {
	...
	private BigInteger sizeOf = BigInteger.valueOf(Integer.MAX_VALUE);
	 ...
	 * **/
	private BigInteger sizeOfProjections = BigInteger.valueOf(Integer.MAX_VALUE);
	
	/** The minimum among sizeOf and sizeOfProjections */
	private BigInteger size = BigInteger.valueOf(Integer.MAX_VALUE);
	
	/** Counts the number of independence assumptions made in the current annotated plan */
	private Integer independenceAssumptions = 0; 
	...
	/** The coverage of the annotated plan w.r.t. the input query **/
	...
	public BigInteger getSizeOfProjection() {
		return this.sizeOfProjections;
	}

	public void setSizeOfProjection(BigInteger size) {
		this.sizeOfProjections = size;
		if(this.size.compareTo(size) > 0) {
			this.size = size;
		}
	}
	
	public BigInteger getSizeOf() {
		return this.sizeOf;
	}

	public void setSizeOf(BigInteger size) {
		this.sizeOf = size;
		if(this.size.compareTo(size) > 0) {
			this.size = size;
		}
	...
	/**
	 * @return the independenceAssumptions
	 */
	public Integer getIndependenceAssumptions() {
		return this.independenceAssumptions;
	}
	
	public void setIndependenceAssumptions(Integer increment) {
		this.independenceAssumptions = increment;
	}
