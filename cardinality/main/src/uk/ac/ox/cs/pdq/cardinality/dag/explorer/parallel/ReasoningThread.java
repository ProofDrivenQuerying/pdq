...
public class ReasoningThread implements Callable<Boolean> {
	...
	protected DAGAnnotatedPlan merge(DAGAnnotatedPlan left, DAGAnnotatedPlan right) {	
		...
		if(representative == null) {
			...
			Triple<BigInteger, Double,Integer> estimate = this.cardinalityEstimator.sizeQualityOf(left, right, this.chaser, this.detector, 
					this.schema.getKeyDependencies());
			configuration.setSizeOf(estimate.getLeft());
			configuration.setQuality(estimate.getMiddle());
			configuration.setIndependenceAssumptions(estimate.getRight());
			//Estimate the configuration's size after considering any projections
		...
		//otherwise, re-use the state of the representative
			...
			//log.trace("Representative " + representative + " for " + left + " " + right);
			configuration = new BinaryAnnotatedPlan(left,right, representative.getState().clone());	
			Triple<BigInteger, Double, Integer> estimate = this.cardinalityEstimator.sizeQualityOf(left, right, this.chaser, this.detector, 
					this.schema.getKeyDependencies());
			configuration.setSizeOf(estimate.getLeft());
			configuration.setQuality(estimate.getRight());	
			configuration.setIndependenceAssumptions(estimate.getRight());
			//Estimate the configuration's size after considering any projections
			BigInteger cardinality = this.cardinalityEstimator.sizeOfProjection(configuration, this.query);
			configuration.setSizeOfProjection(cardinality);		
		}		
