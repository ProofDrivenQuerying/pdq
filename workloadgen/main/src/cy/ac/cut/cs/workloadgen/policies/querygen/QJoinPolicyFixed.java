 ...
 */
public class QJoinPolicyFixed extends QJoinPolicy {
   ...
   @Override
   public boolean createJoinConditions(Schema schema, Query query) {

      Table t = selectRandomTable(schema);
      if (!query.addTable(t))
         return false;
      ...
      while (count < numJoins) {
         ...
         if (countFKJoins < numFKJoins) {
            if (addRandomForeignKeyJoin(query)) {
               ++countFKJoins;
               ++count;
            }
         }
         ...
         if (countJKJoins < numJoins - numFKJoins) {
            if (addRandomJoinableKeyJoin(query)) {
               ++countJKJoins;
               ++count;
            }
         }
