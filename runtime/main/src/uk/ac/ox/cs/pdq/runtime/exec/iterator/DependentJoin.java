...
public class DependentJoin extends TupleIterator {
	...
	/**
	 * Each call on the nextTuple function will make one step on the left child and
	 * generate all matching tuples by iterating over the right child from beginning
	 * to the end. In case more then one tuples were generated, the nextTuple
	 * function will return them one by one. In case there are no tuples generated
	 * it will step the left again and repeat until we generated some tuples or
	 * reached the end of the left child.
	 */
	protected Queue<Tuple> nextTuple = new LinkedBlockingQueue<Tuple>();

	public DependentJoin(TupleIterator child1, TupleIterator child2) {
		super(RuntimeUtilities.computeInputAttributesForDependentJoin(child1, child2),
		...
		for (int inputAttributeIndex = 0; inputAttributeIndex < child2
				.getNumberOfInputAttributes(); ++inputAttributeIndex) {
			if (Arrays.asList(child1.getOutputAttributes())
					.contains(child2.getInputAttributes()[inputAttributeIndex])) {
				foundMatch = true;
			}
		}
		...
		int index = 0;
		for (int inputAttributeIndex = 0; inputAttributeIndex < child1
				.getNumberOfInputAttributes(); ++inputAttributeIndex) {
			int position = Arrays.asList(child1.getOutputAttributes())
					.indexOf(child1.getInputAttribute(inputAttributeIndex));
		...
		for (int inputAttributeIndex = 0; inputAttributeIndex < child2
				.getNumberOfInputAttributes(); ++inputAttributeIndex) {
			int position = Arrays.asList(child2.getOutputAttributes())
					.indexOf(child2.getInputAttribute(inputAttributeIndex));
			Assert.assertTrue(position >= 0);
	...
	public boolean hasNext() {
		...
		if (!this.nextTuple.isEmpty()) {
			return true;
		}
		this.nextTuple();
		return !this.nextTuple.isEmpty();
	}
	...
	public Tuple next() {
		...
		Assert.assertTrue(this.open != null && this.open);
		Assert.assertTrue(!this.interrupted);
		Tuple result = this.nextTuple.poll();
	...
	protected void nextTuple() {
		...
		do {
			...
			try {
				if (this.doCache) {
					this.cacheOfTuplesForRightChildBasedOnInputsFromLeftChild.put(
							Pair.of(this.joinId, this.inputTupleForRightChild),
							this.cachedTuplesReceivedFromRightChild);
				}
			} catch (CacheException e) {
				throw new IllegalStateException();
			}
			this.tupleReceivedFromLeftChild = this.children[0].next();
			this.inputTupleForRightChild = this.projectInputValuesForRightChild(this.tupleReceivedFromParent,
					this.tupleReceivedFromLeftChild);
			this.cachedTuplesReceivedFromRightChild = (Deque<Tuple>) this.cacheOfTuplesForRightChildBasedOnInputsFromLeftChild
					.get(Pair.of(this.joinId, this.inputTupleForRightChild));
			if (this.cachedTuplesReceivedFromRightChild == null) {
				...
				this.cachedIterator = new MemoryScan(this.children[1].getOutputAttributes(),
						this.cachedTuplesReceivedFromRightChild);
	...
	public void receiveTupleFromParentAndPassItToChildren(Tuple tuple) {
		...
		if (this.inputPositionsForChild1.length == tuple.size()) {
			this.children[0]
					.receiveTupleFromParentAndPassItToChildren(tuple);
		} else {
			Object[] inputsForLeftChild = RuntimeUtilities.projectValuesInInputPositions(tuple,
					this.inputPositionsForChild1);
			this.children[0]
					.receiveTupleFromParentAndPassItToChildren(this.child1TupleType.createTuple(inputsForLeftChild));
		}
